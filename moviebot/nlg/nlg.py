...
class NLG:
    ...
    def _clarify_CIN(self, CIN, agent_dact):
        ...
        if CIN[Slots.GENRES.value]:
            genres_list = [
                x for x in CIN[Slots.GENRES.value]
            ...
            if len(genres_list) > 0:
                genres = ', '.join(
                    genres_list[:-1]) + ' and ' + genres_list[-1] if len(
                        genres_list) > 1 else genres_list[-1]
            if genres:
                response = response + ' ' + genres
        if CIN[Slots.KEYWORDS.value] and CIN[
                Slots.KEYWORDS.value] not in Values.__dict__.values():
        ...
        if CIN[Slots.TITLE.value]:
            if not (agent_dact.intent == AgentIntents.RECOMMEND
                ...
                if CIN[Slots.TITLE.value].startswith('.NOT.'):
                    response = response + ' named not similar to ' + CIN[
                        Slots.TITLE.value].replace(negate, '')
                else:
                    response = response + ' named similar to ' + CIN[
                        Slots.TITLE.value]
        if CIN[Slots.DIRECTORS.value] and CIN[
            ...
            if CIN[Slots.DIRECTORS.value].startswith('.NOT.'):
                response = response + ' not directed by ' + CIN[
                    Slots.DIRECTORS.value].replace(negate, '')
            else:
                response = response + ' directed by ' + CIN[
                    Slots.DIRECTORS.value]
        if CIN[Slots.YEAR.value] and CIN[
                Slots.YEAR.value] not in Values.__dict__.values():
            response = response + ' from the ' + self._summarize_title_year(
                CIN[Slots.YEAR.value])
        if CIN[Slots.ACTORS.value] and CIN[
                Slots.ACTORS.value] not in Values.__dict__.values():
            if CIN[Slots.ACTORS.value].startswith('.NOT.'):
                response = response + ' not starring ' + CIN[
                    Slots.ACTORS.value].replace(negate, '')
            else:
                response = response + ' starring ' + CIN[
                    Slots.ACTORS.value].title()
            ...
            if param.slot == Slots.GENRES.value:
                ...
                if negative:
                    ...
                    options[param_key] = [
                        random.choice([
                            f'Don\'t need movies based on '
                            f'"{value}".', f'No need of {_a_an} "{value}" film.'
                        ])
                    ]
