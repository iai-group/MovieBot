...
class DataBase:
    ...
    def __init__(self, path):
        ...
        self.sql_connection = sqlite3.connect(self.db_file_path)  # SQL connection required to
        # access the database
    ...
    def database_lookup(self, dialogue_state, ontology):
        ...
        :param ontology: ontology to check specific parameters
        :return: the results of the SQL query
        """
        if dialogue_state.isBot: # restart the SQL connection if the app is running as a
            # client-server app
            self.sql_connection = sqlite3.connect(self.db_file_path)  # SQL connection required
                ...
                if slot in ontology.multiple_values_CIN:
                    for value in values:
                        if value and value not in Values.__dict__.values():
                            args.append(slot + ' ' + self._get_value_for_query(slot, value))
                elif values and values not in Values.__dict__.values():
                    args.append(slot + ' ' + self._get_value_for_query(slot, values))
    ...
    def _remove_title_from_CIN(self, query_result, condition, args):
        ...
        if len(query_result) == 0 and any([arg.startswith(Slots.TITLE.vlaue) for arg in args]):
            condition = ' AND '.join(
                [arg for arg in args if not arg.startswith(Slots.TITLE.vlaue)]) if len(
                args) > 0 else None
            sql_cursor = self.sql_connection.cursor()
            sql_command = f'SELECT * FROM {self.db_table_name}'
            if condition:
