...
class DialogueState:
    ...
    def __init__(self, ontology, slots, isBot):
        ...
        :param ontology: the ontology of the domain
        :param slots: the slots to find information needs
        ...
        self.frame_PIN = {}  # previous information needs of the user in case user want to go back
    ...
    def _agent_offer_state(self):
        offer_state = {'agent_req_filled': self.agent_req_filled,
    ...
    def _dict(self):
        ...
        :return: a string having basic info about the state
        """
        dstate = {}
        dstate['Previous_Information_Need'] = str({x: self.frame_PIN[x]
                                                   for x, y in self.frame_PIN.items() if y})
        dstate['User_Dialogue_Acts'] = str([str(x) for x in self.last_user_dacts]) if \
            self.last_user_dacts else None
        dstate['Current_Information_Needs'] = str({x: self.frame_CIN[x]
                                                   for x, y in self.frame_CIN.items() if y})
        dstate['Agent_Dialogue_Acts'] = str([str(x) for x in self.last_agent_dacts]) if \
            self.last_agent_dacts else None
    ...
    def initialize(self):
        ...
        self.agent_requestable = deepcopy(self.ontology.agent_requestable)
        self.user_requestable = deepcopy(self.ontology.user_requestable)
        self.agent_req_filled = False  # flag if the necessary information needs are filled
        self.agent_can_lookup = False  # flag if agent can offer based even if agent requirements
        # are not filled
        self.agent_made_partial_offer = False  # flag indicating if agent has results but needs to
        # narrow it further down
        self.agent_should_make_offer = False  # flag indicating if agent is going to make an offer
        self.agent_should_offer_similar = False
        self.similar_movies = {}
        self.agent_made_offer = False  # flag indicating if agent has made recommendation and it
        # is being considered
        self.agent_offer_no_results = False  # flag indicating if agent no results to offer
        self.at_terminal_state = False  # the dialogue must end now
        # Assigning parameters for database results
        self.agent_must_clarify = False  # flag if the same annotation is detected for two slots
        self.dual_params = {}
        self.database_result = None
        self.max_db_result = 100
        self.slot_left_unasked = 3  # number of CIN slots which remain empty before agent must make
        # an offer
