...
class DataLoader:
    ...
    def load_tag_words(self, file_path):
        ...
        :return: The output dictionary extracted from the file
        """
        if os.path.isfile(file_path):
            with open(file_path) as file:
                tag_words = json.load(file)
        else:
            raise FileNotFoundError('File "{}" for tag words not found.'.format(file_path))
        return tag_words
    ...
    def load_database(self):
        ...
        slot_values = {}
        all_data = cursor.execute("Select * from " + db_table_name + ";").fetchall()
        total_count = round(len(all_data), -2)
        print_count = int(total_count / 4)
        self.slots = [x[0] for x in cursor.description if x[0] != Slots.ID.value]
        count = 0
        ...
        for row in all_data:
            ...
            for slot, value in slot_value_pair.items():
                if slot not in self.ontology.slots_annotation: continue
                if slot not in slot_values: slot_values[slot] = {} if slot != Slots.YEAR.value \
                    else []
                if slot in [x.value for x in [Slots.GENRES, Slots.KEYWORDS,
                                              Slots.ACTORS, Slots.DIRECTORS]]:
                    if slot not in slot_values: slot_values[slot] = {}
                    temp_result = [x.strip() for x in value.split(',')]
                    if slot == Slots.GENRES.value: temp_result = [x.lower() for x in temp_result]
                    for temp_value in temp_result:
                        if temp_value not in slot_values[slot]: slot_values[slot].update(
                    ...
                    if value not in slot_values[slot]:
                        if slot == Slots.YEAR.value:
                            ...
                            slot_values[slot].update({value: self.lemmatize_value(value)})
