...
train a dialogue policy. Note that the NLU component of MovieBot is not used in
this environment."""
...
class DialogueEnvMovieBotNoNLU(gym.Env):
    def __init__(
        self,
        ...
        Args:
            ...
            turn_penalty: Penalty for each turn in the dialogue. Defaults to
              0.0.
            b_use_intents: Whether to use intents. Defaults to False.
        """
    ...
    def step(
        ...
        try:
            ...
            agent_utterance, _ = self.agent.generate_utterance(
                agent_dacts,
        ...
        if not done:
            ...
            try:
                ...
                # Get user dialogue act directly from the simulator's
                # annotations
                user_dacts = self._convert_usersim_annotations_to_dact(
                    user_utterance
                )
                ...
                dst = self.agent.dialogue_manager.dialogue_state_tracker
                dst.update_state_user(user_dacts)
            except Exception as e:
                logging.error(e, exc_info=True)
                truncated = True
                reward = 0.0
    ...
    def _save_dialogue_history(self) -> None:
        ...
        Code updated from DialogueKit's DialogueConnector."""
    ...
    def _convert_usersim_annotations_to_dact(
        self, user_utterance: AnnotatedUtterance
    ) -> List[DialogueAct]:
        """Converts the annotations from the user simulator to dialogue acts.
        ...
        """
        user_dacts = []
        user_intent = UserIntents.UNK

        intent_label = user_utterance.intent.label
        for ui in UserIntents.__members__.values():
            if ui.value.label.lower() == intent_label.lower():
                user_intent = ui
                break

        annotations: List[ItemConstraint] = []
        for annotation in user_utterance.annotations:
            slot = annotation.slot
            value = annotation.value
            moviebot_slot = self._find_matching_slot(slot)
            annotations.append(
                ItemConstraint(
                    slot=moviebot_slot if moviebot_slot else slot,
                    op=Operator.EQ,
                    value=value,
                )
            )
    ...
    def _find_matching_slot(self, slot: str) -> Optional[str]:
        """Finds a matching slot in the moviebot domain.

        Args:
            slot: User simulator slot for which to find a correspondence.

        Returns:
            The matching slot in the moviebot domain.
        """
        slot = WNL.lemmatize(slot.lower())
        for moviebot_slot in Slots.__members__.values():
            if slot == "keyword" or slot == "keywords":
                return Slots.KEYWORDS.value
